...
export function fetchUserProfile(usedId) {
  return (dispatch) => {
    let url = `https://backend-deploy-1-8qp1.onrender.com/user/getProfile?user=${usedId}`;
...
export function updateUserProfile(field, data) {
  return (dispatch) => {
    let url = `https://backend-deploy-1-8qp1.onrender.com/user/updateProfile?user=${field}`;
    let token = localStorage.getItem("token");
    let options = {
      method: "POST",
      headers: {
